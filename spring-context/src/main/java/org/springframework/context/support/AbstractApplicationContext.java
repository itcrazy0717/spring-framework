...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			// 创建并初始化BeanFactory
			// [该步骤中的内容有点多，会组装BeanDefinition，并注册到IOC容器中，但是这是针对xml形式的注入]
			...
			try {
				...
				// 激活各种BeanFactory处理器，比如配置文件中占位符的赋值就是在此步骤中进行
				// @Configuration注解的解析就是在此步骤中进行的
				...
				// ApplicationEventListener就是在此方法中添加
				registerListeners();

				// Instantiate all remaining (non-lazy-init) singletons.
				// 初始化单例Bean(非延迟加载的) 注意一般的bean都是非延迟加载的，所以会在IOC容器中缓存该bean的实例
	...
	/**
	 * 实例化并调用已经注入的BeanPostProcessor<br/>
	 * 必须在bean实例化之前调用<br/>
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		...
		// Instantiate all remaining (non-lazy-init) singletons.
		// 初始化单例bean(非延迟初始化) 也就是lazyInit=false的，lazyInit默认值就是false
