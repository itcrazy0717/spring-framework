...
public class ServletInvocableHandlerMethod extends InvocableHandlerMethod {
	...
	public void invokeAndHandle(ServletWebRequest webRequest, ModelAndViewContainer mavContainer,
        ...
        // 设置ModelAndViewContainer为请求已处理，返回
		if (returnValue == null) {
		...
		// 有responseStatusReason
		else if (StringUtils.hasText(getResponseStatusReason())) {
			mavContainer.setRequestHandled(true);
			return;
		}
        // 设置ModelAndViewContainer为请求未处理
		mavContainer.setRequestHandled(false);
		Assert.state(this.returnValueHandlers != null, "No return value handlers");
		// 通过返回值处理器对返回值进行操作，主要是write操作
		try {
	...
	private void setResponseStatus(ServletWebRequest webRequest) throws IOException {
		// 获得状态码
		// 这里如果想要非空，需要通过@ResponseStatus注解进行设置
		...
		if (response != null) {
			...
			if (StringUtils.hasText(reason)) { // 有reason，则设置reason+status
				response.sendError(status.value(), reason);
			}
			else {
				// 无reason，则只设置status即可
				response.setStatus(status.value());
			}
		}

		// To be picked up by RedirectView
		// 为了RedirectView，所以计量设置
