...
public class DispatcherServlet extends FrameworkServlet {
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 如果未开启探测功能，则获得HANDLER_MAPPING_BEAN_NAME对应的bean对象，并设置为handlerMappings
		else {
			try {
	...
	private void initHandlerExceptionResolvers(ApplicationContext context) {
		// 置空handlerExceptionResolvers处理器
		this.handlerExceptionResolvers = null;

		// 自动扫描所有的HandlerExceptionResolver类型
		...
		// 获得名字为handlerExceptionResolver的bean
		else {
			try {
	...
	protected <T> List<T> getDefaultStrategies(ApplicationContext context, Class<T> strategyInterface) {
		...
		if (value != null) {
			// 基于","分割，创建classNames数组
			String[] classNames = StringUtils.commaDelimitedListToStringArray(value);
			// 创建strategies集合
			List<T> strategies = new ArrayList<>(classNames.length);
			// 遍历classNames数组，创建对应的类，添加到strategies集合中
	...
	protected ModelAndView processHandlerException(HttpServletRequest request, HttpServletResponse response,
		...
		// 生成了ModelAndView对象，进行返回
		if (exMv != null) {
			// 如果ModelAndView对象为空，则返回null
			if (exMv.isEmpty()) {
	...
	protected void render(ModelAndView mv, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		// 从request中获得Local对象，并设置到response中
		Locale locale =
